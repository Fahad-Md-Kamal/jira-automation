#!/usr/bin/env python3
"""Packaged Jira Cloud CLI for comments, worklogs, and ticket helpers."""
...
HELP_EPILOG = """examples:
  ...
  jira-push-hook list-issues P1732 --max 50
  jira-push-hook list-assignee --me --project P1732 --max 50
  jira-push-hook comment P1732-800 "Implemented validation changes."
  jira-push-hook worklog P1732-800 30m "Development work." --started "2026-05-19 11:00"
  jira-push-hook ai-contribution P1732-800 25
  jira-push-hook percent-done P1732-800 75
  jira-push-hook transition P1732-800 "In Progress"
  jira-push-hook transition P1732-800 --list
  echo "multi-line comment" | jira-push-hook comment P1732-800
"""

AI_CONTRIBUTION_FIELD_NAME = "AI contribution"
PERCENT_DONE_FIELD_NAME = "%Done"

REQUIRED_SCOPE_HELP = """Required Jira scopes for this CLI:
    SCOPES
...
If field lookup fails, export these environment variables:
...
def get_transitions(issue_key: str) -> list[dict[str, Any]]:
    issue = quote(issue_key, safe="")
    data = jira_request("GET", f"/rest/api/3/issue/{issue}/transitions")
    return data.get("transitions", [])


def resolve_transition_id(transitions: list[dict[str, Any]], target: str) -> str:
    ...
    for transition in transitions:
        if str(transition.get("name", "")).casefold() == normalized:
            return str(transition["id"])

    for transition in transitions:
        if normalized in str(transition.get("name", "")).casefold():
            return str(transition["id"])

    choices = ", ".join(
        f"{item.get('name')} ({item.get('id')})" for item in transitions
    )
    raise JiraError(f"unknown transition {target!r}. Available: {choices}")
...
def print_transitions(transitions: list[dict[str, Any]]) -> None:
    ...
    print("----  -------------------------")
    for transition in transitions:
        print(f"{transition.get('id', ''):<4}  {transition.get('name', '')}")


def cmd_transition(args: argparse.Namespace) -> None:
    transitions = get_transitions(args.issue)
    ...
    target = args.to
    if not target:
        raise JiraError("provide transition name/id via --to or use --list")

    transition_id = resolve_transition_id(transitions, target)
    payload = {"transition": {"id": transition_id}}

    if args.dry_run:
        print("DRY RUN transition payload:")
        print_json(payload)
        return

    issue = quote(args.issue, safe="")
    jira_request("POST", f"/rest/api/3/issue/{issue}/transitions", payload)
    print(f"Transitioned {args.issue} using transition id {transition_id}")
...
def interactive_transition() -> None:
    issue = prompt_required("Ticket number, for example P1732-800: ")
    print("Available transitions:")
    cmd_transition(argparse.Namespace(issue=issue, to=None, list=True, dry_run=False))
    target = prompt_required("Transition name or id: ")
    cmd_transition(argparse.Namespace(issue=issue, to=target, list=False, dry_run=False))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    worklog.add_argument("--started", help="start time, for example '2026-05-19 11:00'")
    ...
    percent_done = subparsers.add_parser("percent-done", help="update the '%Done' custom field")
    ...
    transition = subparsers.add_parser("transition", help="change issue status via Jira transition")
    transition.add_argument("issue", help="issue key, for example P1732-800")
    transition.add_argument("--to", help="transition name or id, for example 'In Progress' or 31")
    transition.add_argument("--list", action="store_true", help="list available transitions for the issue")
    transition.add_argument("--dry-run", action="store_true", help="show transition payload only")
